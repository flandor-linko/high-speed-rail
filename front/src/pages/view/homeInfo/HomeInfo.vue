    ...
    methods: {
        async getRemainInfo() {
            ...
            if (res0 && res0.data && res0.data.status === 200) {
                this.data[0].list = res0.data.data.map(info => `${info.name}应在${moment(info.nextFixTime).format("YYYY-MM-DD hh:mm:ss")}检查`);
            }
            const res1 = await http.get("/demo/device/recentMaintenanceReminder.json");
            if (res1 && res1.data && res1.data.status === 200) {
                this.data[1].list = res0.data.data.map(info => `${info.name}应在${moment(info.nextFixTime).format("YYYY-MM-DD hh:mm:ss")}养修`);
            }
        }
    }
}
